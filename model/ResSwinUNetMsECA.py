...
class ResSwinUNetMsECA(nn.Module):
    def __init__(self, n_channel, n_classes=3):
        super(ResSwinUNetMsECA, self).__init__()
        self.inc = InConv(n_channel, 64)
        self.down1 = Down(64, 128)
        self.down2 = Down(128, 256)
        self.down3 = Down(256, 512)
        self.down4 = Down(512, 512)
        self.up1 = Up(1024+384, 256)
        self.up2 = Up(512+192, 128)
        self.up3 = Up(256+96, 64)
        self.up4 = Up(128, 64)
    ...
    def forward(self, x):
        ...
        x = self.up1(torch.cat([x5, x_t3], dim=1), x4)
        x = self.up2(torch.cat([x, x_t2], dim=1), x3)
        x = self.up3(torch.cat([x, x_t1], dim=1), x2)
        x = self.up4(x, x1)
        x = self.outc(x)
        return x
if __name__ == "__main__":
    ...
    print(summary(ResSwinUNetMsECA(1, 1).to(device), input_size=(1, 256, 256), batch_size=-1))
# end main