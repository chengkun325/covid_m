...
class GhostBottleneck(nn.Module):
    ...
    def __init__(self,
        ...
        mid_chs = int(planes * (base_width / 64.)) * groups
        ...
        self.ghost1 = GhostModule(inplanes, mid_chs, relu=True)
        ...
        self.ghost2 = GhostModule(mid_chs, planes, relu=False)
        ...
        if (inplanes == planes and self.stride == 1):
            ...
            self.shortcut = nn.Sequential(
                nn.Conv2d(inplanes, inplanes, dw_kernel_size, stride=stride,
                          padding=(dw_kernel_size-1)//2, groups=inplanes, bias=False),
...
class Bottleneck(nn.Module):
    ...
    def __init__(
        ...
        # 特征图尺度不变，用HSBlock
            ...
            self.hsblock = GhostBottleneck(width, width*2, width)
...
def Ghost_ResNet50(small_input=True, **kwargs: Any) -> ResNet:
    ...
    """
    return _resnet(GhostBottleneck, [3, 4, 6, 3], small_input, **kwargs)
...
if __name__ == '__main__':
    ...
    model = Ghost_ResNet50(n_channel=1, n_classes=1).cuda().train()
    result = model(img)
    print(result.shape)

    # from torchstat import stat
    # model = Ghost_ResNet50(n_channel=1, n_classes=1)
