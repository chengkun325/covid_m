...
class PagFM(nn.Module):
    ...
    def forward(self, x, y):
        ...
        if self.with_channel:
            sim_map = torch.sigmoid(self.up(x * y))
        else:
            sim_map = torch.sigmoid(torch.sum(x * y, dim=1).unsqueeze(1))
...
class DESegNet7(nn.Module):
    def __init__(self, n_channel, n_classes=3):
        super(DESegNet7, self).__init__()
        self.inc = InConv(n_channel, 64)
        self.down1 = Down(64, 128)
        self.down2 = Down(128, 256)
        self.down3 = Down(256, 512)
        self.down4 = Down(512, 512)
        self.pagfm1 = PagFM(512, 512)
        self.pagfm2 = PagFM(512, 512)
        self.pagfm3 = PagFM(256, 256)
        self.conv1 = nn.Conv2d(384, 512, 3, padding=1)
        self.conv2 = nn.Conv2d(192, 512, 3, padding=1)
        self.conv3 = nn.Conv2d(96, 256, 3, padding=1)
    ...
    def forward(self, x):
        ...
        x_t3 = self.conv1(x_t3)
        x_5 = self.pagfm1(x5, x_t3)

        x_t2 = self.conv2(x_t2)
        x_4 = self.pagfm2(x4, x_t2)
        
        x = self.up1(x_5, x_4)
        
        x_t1 = self.conv3(x_t1)
        x_3 = self.pagfm3(x3, x_t1)
