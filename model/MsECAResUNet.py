...
class MultiScaleConv(nn.Module):
    def __init__(self, in_channel, out_channel):
        ...
        self.conv3_3 = nn.Conv2d(self.single_ch, self.expansion * self.single_ch, kernel_size=3, stride=1, padding=1)
        
        self.atrous_conv3 = nn.Conv2d(self.single_ch, self.expansion * self.single_ch, 3, 1, padding=3, dilation=3)
        self.atrous_conv5 = nn.Conv2d(self.single_ch, self.expansion * self.single_ch, 3, 1, padding=5, dilation=5)
        self.atrous_conv7 = nn.Conv2d(self.single_ch, self.expansion * self.single_ch, 3, 1, padding=7, dilation=7)
        
        self.eca = ECABlock(self.expansion * self.single_ch)
    ...
    def forward(self, x):
        ...
        x2 = self.atrous_conv5(x2)
        x3 = self.atrous_conv7(x3)
        x0 = self.eca(x0)
        x1 = self.eca(x1)
        x2 = self.eca(x2)
        x3 = self.eca(x3)
...
class ResBlock(nn.Module):
    def __init__(self,in_channels,out_channels,stride=[1,1,1],padding=[0,1,0],first=False) -> None:
        ...
        self.bottleneck = nn.Sequential(
            nn.Conv2d(in_channels,2*in_channels,kernel_size=1,stride=stride[0],padding=padding[0],bias=False),
            nn.BatchNorm2d(2*in_channels),
            nn.ReLU(inplace=True), # 原地替换 节省内存开销
            MultiScaleConv(2*in_channels, 2*in_channels),
            # nn.Conv2d(2*in_channels,out_channels,kernel_size=3,stride=stride[1],padding=padding[1],bias=False),
            nn.BatchNorm2d(2*in_channels),
            nn.ReLU(inplace=True), # 原地替换 节省内存开销
            nn.Conv2d(2*in_channels,out_channels,kernel_size=1,stride=stride[2],padding=padding[2],bias=False),
...
class InConv(nn.Module):
    def __init__(self, in_channel, out_channel):
        ...
        self.conv1 = ResBlock(32, out_channel)
...
class MsECAResUNet(nn.Module):
    def __init__(self, n_channel, n_classes=3):
        super(MsECAResUNet, self).__init__()
...
if __name__ == "__main__":
    ...
    print(summary(MsECAResUNet(4, 1).to(device), input_size=(4, 224, 224), batch_size=-1))
