...
class ResSwinUNetMsECA1(nn.Module):
    def __init__(self, n_channel, n_classes=3):
        super(ResSwinUNetMsECA1, self).__init__()
        ...
        self.conv2 = nn.Conv2d(192, 256, 1)
        self.conv3 = nn.Conv2d(96, 128, 1)
    ...
    def forward(self, x):
        ...
        x = self.up1(x5+x_t3, x4)
        x = self.up2(x+x_t2, x3)
        x = self.up3(x+x_t1, x2)
        x = self.up4(x, x1)
        x = self.outc(x)
        return x
if __name__ == "__main__":
    # img = torch.randn(1, 1, 256, 256).cuda()
    # model = ResSwinUNetMs(1, 1).cuda()
    # y = model(img)
    # print(y.size())
    from torchsummary import summary

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(summary(ResSwinUNetMsECA1(1, 1).to(device), input_size=(1, 256, 256), batch_size=-1))
# end main