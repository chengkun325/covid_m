...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        self.mlp = MixFFN_skip(dim, mlp_hidden_dim)
...
class SwinTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.pos_drop(x)
        x0 = x.permute(0, 2, 1).reshape(B, -1, H//4, W//4)
        x = self.layer1(x)
        x1 = x.permute(0, 2, 1).reshape(B, -1, H//8, W//8)
        x = self.layer2(x)
        x2 = x.permute(0, 2, 1).reshape(B, -1, H//16, W//16)
        return x0, x1, x2
...
class ResDoubleConv(nn.Module):
    def __init__(self, in_channel, out_channel):
        ...
        self.res1 = ResBlock(in_channel, out_channel, out_channel)
        self.res2 = ResBlock(out_channel, out_channel, out_channel)
...
class ResSwinUNetMsECA4(nn.Module):
    def __init__(self, n_channel, n_classes=3):
        super(ResSwinUNetMsECA4, self).__init__()
        self.inc = InConv(n_channel, 64)
        self.down1 = Down(64, 128)
        self.down2 = Down(128, 256)
        self.down3 = Down(256, 512)
        self.down4 = Down(512, 512)
        self.conv1 = nn.Conv2d(384+512, 512, 1)
        self.eca1 = ECABlock(384+512)
        self.conv2 = nn.Conv2d(192+512, 512, 1)
        self.eca2 = ECABlock(192+512)
        self.conv3 = nn.Conv2d(96+256, 256, 1)
        self.eca3 = ECABlock(96+256)
        self.up1 = Up(1024, 256)
        self.up2 = Up(512, 128)
        self.up3 = Up(256, 64)
        self.up4 = Up(128, 64)
        self.outc = OutConv(64, n_classes)
        self.trans_encoder = SwinTransformer()
    ...
    def forward(self, x):
        ...
        x_t1, x_t2, x_t3 = self.trans_encoder(x)
        x_5 = torch.cat([x_t3, x5], dim=1)
        x_5 = self.eca1(x_5)
        x_5 = self.conv1(x_5)
        x_4 = torch.cat([x_t2, x4], dim=1)
        x_4 = self.eca2(x_4)
        x_4 = self.conv2(x_4)
        # print(x_5.size(), x_4.size())
        x = self.up1(x_5, x_4)
        x_3 = torch.cat([x_t1, x3], dim=1)
        x_3 = self.eca3(x_3)
        x_3 = self.conv3(x_3)
        x = self.up2(x, x_3)
        x = self.up3(x, x2)
...
if __name__ == "__main__":
    ...
    model = ResSwinUNetMsECA4(1, 1).cuda()
    y = model(img)
    print(y.size())
    # from torchsummary import summary

    # device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    # print(summary(ResSwinUNetMsECA2(1, 1).to(device), input_size=(1, 256, 256), batch_size=-1))
# end main