...
class UTNet(nn.Module):
    ...
    def __init__(self, n_channel, n_classes=3, base_chan=32, reduce_size=8, block_list='1234', num_blocks=[1,1,1,1],
                 projection='interp', num_heads=[4, 4, 4, 4], attn_drop=0.1, proj_drop=0.1, bottleneck=False, maxpool=True,
                 rel_pos=False, aux_loss=False):
        super().__init__()

        self.aux_loss = aux_loss
        self.inc = [BasicBlock(n_channel, base_chan)]
        if '0' in block_list:
        ...
        if aux_loss:
            self.out1 = nn.Conv2d(8 * base_chan, n_classes, kernel_size=1, bias=True)
            self.out2 = nn.Conv2d(4 * base_chan, n_classes, kernel_size=1, bias=True)
            self.out3 = nn.Conv2d(2 * base_chan, n_classes, kernel_size=1, bias=True)
...
if __name__ == '__main__':
    ...
    net = UTNet(1,1).cuda()
    size = 224
    img = torch.randn(1, 1, 224, 224).cuda()
    output = net(img)
    print(output.size())
    # device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    # print(summary(net.to(device), input_size=(1, size, size), batch_size=3))