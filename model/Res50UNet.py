...
class MultiScaleConv(nn.Module):
    def __init__(self, in_channel, out_channel, stride, groups):
        ...
        self.conv3_3 = nn.Conv2d(in_channel, self.single_ch, kernel_size=3, stride=stride, groups=groups, padding=1)
        
        self.atrous_conv3 = nn.Conv2d(in_channel, self.single_ch, 3, stride, groups=groups, padding=3, dilation=3)
        self.atrous_conv5 = nn.Conv2d(in_channel, self.single_ch, 3, stride, groups=groups, padding=5, dilation=5)
        self.atrous_conv7 = nn.Conv2d(in_channel, self.single_ch, 3, stride, groups=groups, padding=7, dilation=7)
...
class Bottleneck(nn.Module):
    ...
    def __init__(
        ...
        self.conv2 = MultiScaleConv(width, width, stride, groups)
        self.bn2 = norm_layer(width//4*7)
        self.conv3 = conv1x1(width//4*7, planes * self.expansion)
...
class ResNet(nn.Module):
    def __init__(
        ...
        # self.neckConv = nn.Conv2d(1024, 512, 1, 1)
...
def Res50UNet(n_channel=1, n_classes=1, weights= None, progress: bool = True, **kwargs: Any) -> ResNet:
    return _resnet(Bottleneck, [3, 4, 6, 3], weights, progress, **kwargs)
...
if __name__ == "__main__":
    # model = Res50UNet(n_channel=1, n_classes=3).cuda()
    ...
    print(summary(Res50UNet(n_channel=1, n_classes=1).to(device), input_size=(1, 224, 224), batch_size=-1))
# end main