...
class MiniSeg(nn.Module):
    def __init__(self, n_channel, n_classes=3, P1=2, P2=3, P3=8, P4=6, aux=False, *args, **kwargs):
        ...
        self.long1 = DownsamplerBlock(n_channel, 8, stride=2)
        self.down1 = ConvBlock(n_channel, 8, stride=2)
        ...
        if self.aux:
            self.pred4 = nn.Sequential(nn.Dropout2d(0.01, False), nn.Conv2d(64, n_classes, 1, stride=1, padding=0))
            self.pred3 = nn.Sequential(nn.Dropout2d(0.01, False), nn.Conv2d(32, n_classes, 1, stride=1, padding=0))
            self.pred2 = nn.Sequential(nn.Dropout2d(0.01, False), nn.Conv2d(24, n_classes, 1, stride=1, padding=0))
        self.pred1 = nn.Sequential(nn.Dropout2d(0.01, False), nn.Conv2d(8, n_classes, 1, stride=1, padding=0))
    ...
    def forward(self, input):
        ...
        up0 = F.interpolate(up1,input.size()[2:],mode='bilinear', align_corners=False)
        ...
        pred1 = F.interpolate(self.pred1(up0), input.size()[2:], mode='bilinear', align_corners=False)
...
if __name__ == '__main__':
    ...
    # print(summary(MiniSeg(1, 1).to(device), input_size=(1,224,224), batch_size=-1))
