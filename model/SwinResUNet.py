...
class SwinTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        # x = self.layer3(x)
        # x3 = x.permute(0, 2, 1).reshape(B, -1, H//32, W//32)
...
class ResNet(nn.Module):
    def __init__(
        ...
        self.swin = SwinTransformer(img_size=224, patch_size=4, in_chans=1)
        self.neckConv = ASPP(1408, 512)
        self.up1 = Up(1216, 256)
        self.up2 = Up(608, 128)
        self.up3 = Up(256, 64)
        self.up4 = Up(128, 64)
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        ...
        x_s3, x_s4, x_s5 = self.swin(x)
        x = self.conv1(x)
        x = self.bn1(x)
        x1 = self.relu(x) # 224*224
        x2 = self.maxpool(x1)
        x2 = self.layer1(x2) # 112 * 112
        x3 = self.layer2(x2) # 56 * 56
        x4 = self.layer3(x3) # 28 * 28
        x5 = self.layer4(x4) # 14 * 14
        x5 = torch.cat([x5, x_s5], dim=1)
        x5 = self.neckConv(x5)
        x4 = torch.cat([x4, x_s4], dim=1)
        x = self.up1(x5, x4)
        x3 = torch.cat([x3, x_s3], dim=1)
        x = self.up2(x, x3)
        x = self.up3(x, x2)
        x = self.up4(x, x1)
        x = self.outc(x)
...
def SwinRes50UNet(n_channel=1, n_classes=1, weights= None, progress: bool = True, **kwargs: Any) -> ResNet:
    return _resnet(Bottleneck, [3, 4, 6, 3], weights, progress, **kwargs)

if __name__ == "__main__":
    
    # model = SwinRes50UNet(n_channel=1, n_classes=3).cuda()
    # img = torch.randn(8, 1, 224, 224).cuda()
    # y = model(img)
    # print(y.size())
    
    model = SwinRes50UNet().cuda()
    img = torch.randn(8, 1, 224, 224).cuda()
    y = model(img)
    print(y.size())
    ...
    # print(summary(resnet50().to(device), input_size=(3, 224, 224), batch_size=-1))
# end main