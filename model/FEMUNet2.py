...
class FeatureEnchanceModule(nn.Module):
    def __init__(self, in_channel, out_channel):
        ...
        # 设计自适应卷积核，便于后续做1*1卷积
        kernel_size = int(abs((math.log(in_channel, 2) + 1) / 2))
        kernel_size = kernel_size if kernel_size % 2 else kernel_size + 1
    ...
    def forward(self, x):
        ...
        # 然后，基于1*1卷积学习通道之间的信息；其中，使用前面设计的自适应卷积核
        # v = self.conv(v.squeeze(-1).transpose(-1, -2)).transpose(-1, -2).unsqueeze(-1)
        context = context.squeeze(-1).transpose(-1, -2)
        channel_add_term = self.conv(context)
        channel_add_term = channel_add_term.transpose(-1, -2).unsqueeze(-1)
        channel_add_term = self.sigmoid(channel_add_term)
...
class FEMUNet2(nn.Module):
    def __init__(self, n_channel, n_classes=3):
        super(FEMUNet2, self).__init__()
...
if __name__ == "__main__":
    ...
    print(summary(FEMUNet2(1, 1).to(device), input_size=(1, 256, 256), batch_size=-1))
