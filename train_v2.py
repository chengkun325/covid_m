...
def write_scalar(summary_writer:SummaryWriter, seg_metric:dict, epoch, class_num, mode="train"):
    # 存成scalar，方便使用tensorboard绘图对比
    
    for k, v in seg_metric.items():
#         print(type(v), v)
        if class_num > 2 and isinstance(v, np.ndarray):
            for idx, item in enumerate(v):
                summary_writer.add_scalar("{}/{} - type {}".format(mode, k, idx), item, global_step=epoch)
            
        else:
            summary_writer.add_scalar("{}/{}".format(mode, k), v, global_step=epoch)
...
def train_net(
    ...
    *args, **kwargs
    ...
    criterion = loss_cls(num_classes=class_num, Beta=beta)
    ...
    for epoch in range(1, epochs + 1):
        print("Starting train epoch {}/{}".format(epoch, epochs).center(50, "="))
        ...
        write_scalar(summary_writer, epoch_metric.avg_metric, epoch, class_num)
        # 模型验证
        net.eval()
        epoch_metric = AvgMetricPool()
        ...
        write_scalar(summary_writer, epoch_metric.avg_metric, epoch, class_num, "valid")
        if epoch_metric.avg_metric[best_score_metric] > best_score and epoch > epoch_limit:
            best_score = epoch_metric.avg_metric[best_score_metric]
            torch.save(net.state_dict(),
                       "./checkpoints/{}.pth".format(file_name))
    torch.save(net.state_dict(),
                       "./checkpoints/{}_last.pth".format(file_name))
...
def get_args():
    ...
    parser.add_argument('--beta', type=float,
                        default=0.5, help='loss paramater')
