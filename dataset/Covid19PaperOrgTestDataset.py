...
class Covid19PaperOrgTestDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if np.max(image) > 1.0:
            image = image / 255.0
        if np.max(org_image) > 1.0:
            org_image = org_image / 255.0
        image = np.expand_dims(image, axis=0).astype(np.float32)
        mask = np.expand_dims(mask, axis=0).astype(np.uint8)
        org_image = np.expand_dims(org_image, axis=0).astype(np.float32)
        org_mask = np.expand_dims(org_mask, axis=0).astype(np.uint8)

        return torch.Tensor(image), torch.Tensor(mask), torch.Tensor(org_image), torch.Tensor(org_mask)
